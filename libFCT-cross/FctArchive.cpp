...
#pragma region Constructors & Destructor
	...
	FctArchive::FctArchive(std::string newArchivePath, uint16_t chunkSize) {
		...
		uint8_t fctHeader[ArchiveHeaderSize] = { 'F', 'C', 'T', (uint8_t)ChunkSize, (uint8_t)(ChunkSize >> 8) };
...
#pragma endregion
...
#endif
		else // fall back to save seek which will most likely be able to handle the file
	...
	// basically like extractFiles but everything except the indices is written to a new file
	int FctArchive::removeFiles(std::vector<uint32_t> indices) {
		FILE* tmpFile;
		char* buffer = new char[ChunkSize];
		std::filesystem::path tmpPath = std::filesystem::path(ArchivePath.generic_string() + ".tmp");
		...
		for (uint32_t i = 0; i < indexSize; i++) {
			...
			if (!indices.empty() && i + 1 == indices.back()) { 
				seekFile(file);
				indices.pop_back();
			}
			else {
				fwrite(file.Header.data(), 1, file.Header.size(), tmpFile); // no need to read the header from the file
				fseek(ArchiveFile, file.Header.size(), SEEK_CUR);
				for (uint32_t j = 1; j < file.ChunkCount; j++) {
		...
		std::filesystem::rename(std::filesystem::path(tmpPath), std::filesystem::path(ArchivePath));
	...
	int FctArchive::extractFiles(std::vector<uint32_t> indices) {
		...
		for (uint32_t i = 0; i < FileIndex.size(); i++) {
			...
			if (!indices.empty() && i == indices.back()) {
				...
				FCT::FS::fileOpen(&curFile, file.FormattedFilePath.c_str(), "wb");
				if (curFile == NULL)
					return errno;
				...
				fread(buffer, 1, ChunkSize, ArchiveFile);
				fwrite(buffer, 1, file.LastChunkContentSize, curFile);
	...
	uint16_t FctArchive::getChunkSize() {
		return ChunkSize;
	}
}